...
preflight_source() {
  ...
  if [[ "$missing" -gt 0 ]]; then
    echo ""
    red "Seu clone local nao contem todos os agentes da v2.1. Instalacao abortada."
...
cleanup_legacy_agents() {
  local target_dir="$1"
  local removed=0
  local legacy_agents=("qa-engineer.md" "cybersecurity.md" "devops.md" "backend.md" "frontend.md" "luna-operator.md" "terra-lead.md")
  ...
  if [[ "$removed" -gt 0 ]]; then green "  limpeza concluida: ${removed} agente(s) legado(s) removido(s)"; fi
}

verify_installation() {
  local root="$1" agents_dir="$2" skills_dir="$3" errors=0
  echo ""
  blue "Verificando instalacao efetiva"
  if [[ ! -f "${root}/opencode.json" ]]; then
    red "  AUSENTE: ${root}/opencode.json"; errors=$((errors + 1))
  elif ! grep -q '"default_agent"[[:space:]]*:[[:space:]]*"luna-lead"' "${root}/opencode.json"; then
    red "  opencode.json instalado nao aponta para luna-lead"; errors=$((errors + 1))
  else
    green "  default_agent: luna-lead"
  fi
  for name in "${EXPECTED_AGENTS[@]}"; do
    if [[ -f "${agents_dir}/${name}.md" || -L "${agents_dir}/${name}.md" ]]; then green "  agente instalado: ${name}"; else red "  agente AUSENTE no destino: ${name}"; errors=$((errors + 1)); fi
  done
  for name in "${EXPECTED_SKILLS[@]}"; do
    if [[ -f "${skills_dir}/${name}/SKILL.md" || -L "${skills_dir}/${name}/SKILL.md" ]]; then green "  skill instalada: ${name}"; else red "  skill AUSENTE no destino: ${name}"; errors=$((errors + 1)); fi
  done
  if [[ "$errors" -gt 0 ]]; then
    echo ""
...
if [[ "$MODE" == "project" ]]; then
  ROOT="$(pwd)"; AGENTS_DIR="${ROOT}/.opencode/agents"; SKILLS_DIR="${ROOT}/.opencode/skills"
  ...
  ROOT="$GLOBAL_DIR"; AGENTS_DIR="${GLOBAL_DIR}/agents"; SKILLS_DIR="${GLOBAL_DIR}/skills"
...
if bash "${REPO_DIR}/scripts/validate.sh"; then green "Configuracao instalada e validada."; else red "A validacao encontrou problemas."; exit 1; fi
